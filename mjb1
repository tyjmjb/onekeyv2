...
v2ray_conf_add_tls(){
    cd /etc/v2ray
    wget --no-check-certificate https://raw.githubusercontent.com/tyjmjb/onekeyv2/master/v2ray.json -O v2ray.json
    modify_path
    modify_alterid
    modify_inbound_port
}
...
install_v2ray_ws_tls(){
    ...
    enable_process_systemd
    acme_cron_update
    echo -e "基本安装完成，请继续到/etc/v2ray/目录下修改config.json才能使用"
    sleep 2
}
...
menu(){
    ...
    echo -e "【半自动】安装后，请做两件事情："
    echo -e "1，修改您的 /etc/v2ray/config.json 里面的服务器信息"
    echo -e "2，使用screen 启动，具体见帮助"
    echo -e ""
    echo -e "—————————————— 其他选项 ——————————————"
