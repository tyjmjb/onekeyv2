...
nginx_conf="${nginx_conf_dir}/conf.d/v2ray.conf"
nginx_dir="/etc/nginx"
...
nginx_exist_check(){
    ...
    yum -y install nginx jemalloc 
    echo -e "${OK} ${GreenBG} 魔改版：执行旧脚本的配置中${Font}"
    ...
    then
        ...
        echo -e  "${OK} ${GreenBG} Error!!! Nginx安装失败，请手动执行安装nginx后重试，方法是yum -y install nginx或者apt-get install nginx${Font}"
        exit 1
    fi
}
nginx_install(){
    echo -e "${OK} ${GreenBG} Error错误，脚本出错，nginx安装失败请手动安装nginx${Font}"
    exit 1
}
ssl_install(){
    if [[ "${ID}" == "centos" ]];then
...
nginx_conf_add(){
    touch ${nginx_conf_dir}/conf.d/v2ray.conf
    cat>${nginx_conf_dir}/conf.d/v2ray.conf<<EOF
    server {
...
nginx_systemd(){
echo -e "${OK} ${GreenBG} 魔改版跳过systemd file修改 ${Font}"
sleep 2
}
tls_type(){
echo -e "${OK} ${GreenBG} 魔改版自动兼容TLS1.1 TLS1.2 TLS1.3 ${Font}"
sleep 2
}
show_access_log(){
